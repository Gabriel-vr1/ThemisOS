    ...
    <div className="flex flex-col gap-6">
      <section className="rounded-xl border border-[#202B43] bg-[#121A2B] p-5 shadow-[0_18px_50px_rgba(0,0,0,0.22)]">
        <div className="max-w-3xl">
          <p className="text-xs font-semibold uppercase tracking-wider text-[#C6A664]">Research Hub</p>
          <h1 className="mt-2 text-2xl font-semibold text-[#F3F0E8]">AI governance knowledge base</h1>
      ...
      {filteredSections.length > 0 ? (
        filteredSections.map(section => (
          <section key={section.id} className="rounded-xl border border-[#202B43] bg-[#121A2B] p-5 shadow-[0_18px_50px_rgba(0,0,0,0.22)]">
            <h2 className="text-sm font-semibold uppercase tracking-wider text-[#8E96A8]">{section.label}</h2>
            <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {section.items.map(item => (
                <article key={`${section.id}-${item.title}`} className="rounded-lg border border-[#202B43] bg-[#0B1020]/60 p-4 shadow-[inset_0_1px_0_rgba(243,240,232,0.035)]">
                  <div className="flex items-start justify-between gap-3">
                    <h3 className="text-sm font-semibold text-[#F3F0E8]">{item.title}</h3>
                    {item.color && (
                      <span className="mt-1 h-3 w-3 shrink-0 rounded-full" style={{ background: item.color }} />
                    )}
                  </div>
                  {item.meta && <p className="mt-2 text-xs text-[#D8BC7A]">{item.meta}</p>}
                  <p className="mt-3 text-sm leading-relaxed text-[#8E96A8]">{item.body}</p>
                  <div className="mt-4 flex flex-wrap gap-1.5">
                    {item.tags.map(tag => (
                      <span key={tag} className="rounded-full border border-[#202B43] px-2 py-1 text-xs text-[#8E96A8]">
                        {tag}
                      </span>
                    ))}
                  </div>
        ...
        <section className="rounded-xl border border-[#202B43] bg-[#121A2B] p-8 shadow-[0_18px_50px_rgba(0,0,0,0.22)] text-center">
          <p className="text-sm font-semibold text-[#F3F0E8]">No research notes found</p>
          <p className="mt-2 text-xs text-[#8E96A8]">Try another search term or clear the topic filter.</p>
        </section>
      )}
    </div>
  )
