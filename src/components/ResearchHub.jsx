    ...
    <div className="flex flex-col gap-6">
      <section className="rounded-xl border border-gray-800 bg-gray-900 p-5">
        <div className="max-w-3xl">
          <p className="text-xs font-semibold uppercase tracking-wider text-blue-400">Research Hub</p>
          <h1 className="mt-2 text-2xl font-semibold text-white">AI governance knowledge base</h1>
      ...
      {filteredSections.length > 0 ? (
        filteredSections.map(section => (
          <section key={section.id} className="rounded-xl border border-gray-800 bg-gray-900 p-5">
            <h2 className="text-sm font-semibold uppercase tracking-wider text-gray-400">{section.label}</h2>
            <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {section.items.map(item => (
                <article key={`${section.id}-${item.title}`} className="rounded-lg border border-gray-800 bg-gray-950/60 p-4">
                  ...
                  {item.meta && <p className="mt-2 text-xs text-blue-300">{item.meta}</p>}
                  <p className="mt-3 text-sm leading-relaxed text-gray-400">{item.body}</p>
                  <div className="mt-4 flex flex-wrap gap-1.5">
                    {item.tags.map(tag => (
                      <span key={tag} className="rounded-full border border-gray-700 px-2 py-1 text-xs text-gray-400">
                        {tag}
                      </span>
                    ))}
                  </div>
        ...
        <section className="rounded-xl border border-gray-800 bg-gray-900 p-8 text-center">
          <p className="text-sm font-semibold text-white">No research notes found</p>
          <p className="mt-2 text-xs text-gray-400">Try another search term or clear the topic filter.</p>
        </section>
      )}
    </div>
  )
